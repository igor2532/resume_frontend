...
const exportPdf = () => {
  ...
  Условия:{' '}
  {Array.isArray(cv?.conditions) && cv.conditions.length > 0 ? (
    ...
    <span style={{ color: '#888' }}>не указаны</span>
  )}
</div>
  ...
  {Array.isArray(cv?.jobs) && cv.jobs.length > 0 ? (
    cv.jobs.map((el) => (
...
<div className={styles.cv_jobs}>
  ...
  {Array.isArray(cv?.institutions) && cv.institutions.length > 0 ? (
